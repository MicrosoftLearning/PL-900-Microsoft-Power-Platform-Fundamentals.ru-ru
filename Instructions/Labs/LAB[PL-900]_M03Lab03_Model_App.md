...
- Должно быть выполнено **задание модуля 0 "Проверка лабораторной среды"** .

- Должно быть выполнено **задание модуля 2 "Моделирование данных"** .

- Должно быть выполнено **задание модуля 3 "Создание приложение на основе холста"** .
...
4. В разделе "Возможности данных" выберите **Формы** и щелкните форму "Сведения" с типом **Основная**, чтобы открыть ее.

**ВАЖНО**. По умолчанию всем формам задано имя "Сведения", поэтому обязательно проверьте, что тип выбранной формы — **Основная**, а не какой-то другой. По умолчанию в форме имеется два поля: «Имя» (основное поле) и «Владелец».
...
6. В меню в верхней части экрана выберите **+ Поле формы** и добавьте под полем **Владелец** следующие поля, перетаскивая столбцы в форму или просто щелкая заголовки столбцов:
    ...
    2. **Планируемое окончание**

    3. **Фактическое начало**

    4. **Фактическое окончание**
...
3. Создайте приложение на основе модели, выполнив приведенные ниже действия.
    ...
    0. Выберите **Пустое приложение** в разделе **Начать с** на начальном экране.

    1. В разделе **Пустое приложение на основе Dataverse** нажмите кнопку **Создать**.

    2. Введите **Управление Bellows Campus**  в качестве имени и нажмите кнопку **Создать**.
...
Задача 2. Редактирование приложения
...
1. Запуск приложения
    ...
    0. Выберите **Воспроизвести**, чтобы открыть приложение в новом окне.
...
2. Создайте новый контакт.
    ...
    2. Укажите в поле **Имя** Александр, а в поле **Фамилия** — Демидов.

    3. В поле **Email** укажите свой личный адрес электронной почты. Он будет использоваться в одном из следующих заданий для получения сообщений электронной почты.

    4. Щелкните **Сохранить и закрыть**.

    5. После этого созданный контакт должен отобразиться в представлении **Активные контакты**.

3. Создайте новый визит.

    0. Выберите **Визиты** на карте сайта.

    1. Нажмите кнопку **Создать**.

    2. Введите следующие поля:

        0. **Имя**: Новый тестовый визит

        1. **Посетитель**: Выберите Александра Демидова

        2. **Запланированное начало**: выберите завтрашнее число и время начала 14:00

        3. **Запланированное окончание**: выберите завтрашнее число и время окончания 15:30

...
5.  Выберите **+ Новый поток** и **выберите Автоматизированный облачный поток**.

6.  Введите `Visit Notification` в поле **Имя потока**.
...
9.  Укажите условия триггера для потока, как показано ниже.
    ...
    4.  На шаге триггера нажмите кнопку с многоточием (**...**) и выберите **Переименовать**. Переименование шага триггера `When a Visit is added` 
...
5.  Выберите поле **Идентификатор строки**. Обратите внимание, что появится окно для выбора **динамического содержимого** или **выражений**.

6.  В поле **Идентификатор строки** выберите **Посетитель (значение)** в списке **Динамическое содержимое** . На этом шаге вы ищете контактное лицо для строки с информацией о визите, при создании которой был активирован этот поток. Так как адрес электронной почты находится в таблице контактов, вам нужно получить эту информацию для отправки посетителю сообщения электронной почты.
...
2.  Найдите `mail`, выберите действие **Отправить сообщение электронной почты (версия 2)** в **соединителе Office 365 Outlook**.

3.  Если появится запрос на принятие условий для использования этого действия, выберите **Принять**.
...
3.  Выберите **Приложения** и откройте созданное ранее приложение На основе модели **Bellows Campus Management** .
...
9.  Перейдите на вкладку браузера, на которой выполняется тест Flow. После короткой задержки вы увидите выполнение потока. Здесь вы сможете обнаружить проблемы, если они возникнут в потоке, или подтвердить его успешное выполнение.
...
- Поэкспериментируйте с форматированием в сообщении электронной почты. Как вы можете сделать его более профессиональным?
